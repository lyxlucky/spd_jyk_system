<template>
  <div class="stock-plan-order-list">
    <el-card>
      ...
      <div>
        <el-form class="stock-plan-toolbar" :model="form" :inline="true" size="mini">
          ...
          <el-form-item>
            <el-button type="primary" :disabled="!canAddDetail" @click="addMonitoringVisible = true">
              增加备货明细
            </el-button>
          </el-form-item>
          <el-form-item>
          ...
          <el-form-item>
            <el-button type="primary" @click="handleSendHrpList">发送审批</el-button>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="handleSetOrderType">设置订单类型</el-button>
          </el-form-item>
          <el-form-item>
    ...
    />
    <AddMonitoringDialog
      :visible.sync="addMonitoringVisible"
      :order-row="currentTableData || {}"
...
<script>
  ...
  import { isSaveAddDisabledByHrp, isBatchApproveOnlyHp } from '../utils';
  export default {
    ...
    computed: {
      ...
      canAddDetail() {
        const row = this.currentTableData;
        if (!row || String(row.APPROVE_STATE) !== '0') {
          return false;
        }
        if (isSaveAddDisabledByHrp(row.SENDHRP)) {
          return false;
        }
        return true;
      }
    ...
    methods: {
      ...
      handleSendHrpList() {
        if (this.selectedRows.length === 0) {
          this.$message.warning('请勾选单号');
          return;
        }
        const loading = this.$messageLoading('处理中...');
        upStockToHrpList(this.selectedRows.map((r) => r.STOCK_UP_PLAN_NO))
          .then((res) => {
            const data = res.data;
            if (data.code == 200) {
              this.handleSearch();
            }
            this.$message[data.code == 200 ? 'success' : 'warning'](
              data.msg || '操作完成'
            );
          })
      ...
      handleSetOrderType() {
        ...
        this.$prompt('请选择订单类型：0-普通，1-紧急', '设置订单类型', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          inputValue: '0',
          inputPattern: /^[01]$/,
          ...
          .then(({ value }) => {
            const loading = this.$messageLoading('处理中...');
            setOrderType(
              this.selectedRows.map((r) => r.STOCK_UP_PLAN_NO),
              value
            )
              .then((res) => {
                const data = res.data;
