<template>
  <div class="ele-body">
    <!-- 数据表格 -->
    <AdvanceReceiptNumberDelSearch @search="reload" @handleAddConsumeItem="handleAddConsumeItem"
      @handleUdiScanAdd="handleUdiScanAdd" :ApplyTempTableDataSearch="ApplyTempTableDataSearch"
      :AdvanceReceiptDelcurrent="current" :selection="selection"
      :VarietyConsumeptionDataList="VarietyConsumeptionDataList" />
    <ele-pro-table ref="table" :rowClickCheckedIntelligent="false" :rowClickChecked="true" height="50%"
      :paginationStyle="paginationStyle" highlight-current-row :stripe="true" :pageSize="pageSize"
      :pageSizes="pageSizes" :columns="columns" :datasource="datasource" :selection.sync="selection"
      @selection-change="onSelectionChange" @current-change="onCurrentChange" cache-key="ApplyTempDataTable">
      <!-- 左表头 -->
      ...
      <template v-slot:ACTION="{ row }">
        <el-button size="mini" type="primary" @click="handleUpdateCount(row)">修改数量</el-button>
      </template>
    </ele-pro-table>
    <UDIScanAddDialog :AdvanceReceiptNumberCurrent="ApplyTempTableDataSearch" :AdvanceReceiptDelcurrent="current"
      :visible.sync="UDIScanAddDialogVisiable" @udiScanDialogClose="handleClosed">
    </UDIScanAddDialog>
  </div>
</template>
