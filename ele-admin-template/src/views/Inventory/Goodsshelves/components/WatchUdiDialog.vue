...
import {
  getShUdiData,
  delShUdiData
} from '@/api/Inventory/CentreBankWatchUdi';
...
/**
 * 流向查询专用「查看UDI」弹窗（纯 Vue，接口同收货 UDI）
 */
export default {
  name: 'GoodsshelvesWatchUdiDialog',
  props: {
    visible: Boolean,
  ...
  methods: {
    ...
    onOpen() {
      if (!this.batchId) {
        this.$message.warning('缺少批号标识，无法查询 UDI');
        return;
      }
      this.load(true);
    },
    ...
    async load(resetPage) {
      ...
      try {
        ...
        this.$message.error(e.message || '加载 UDI 失败');
        this.rows = [];
        this.total = 0;
      } finally {
        this.loading = false;
    ...
    async onDelete() {
      ...
      try {
        await this.$confirm('确认要删除此数据吗？', '提示', { type: 'warning' });
        const ids = this.selection
          .map((r) => r.ID)
          .filter(Boolean)
          .join(',');
        this.deleting = true;
        await delShUdiData(ids);
