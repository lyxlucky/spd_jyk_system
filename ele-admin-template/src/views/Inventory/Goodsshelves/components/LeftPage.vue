...
<script>
  ...
  import { GetPDAList } from '@/api/Inventory/Goodsshelves';
  export default {
    name: 'SystemUser',
    components: {
      UserSearch
        ...
        columns: [
          ...
          },
          {
            ...
            formatter: function (row) {
              ...
              return (
                '<div style="text-align:left"><a style="text-align:left" lay-event="GSOURCE_EDIT_lxBZ">' +
                row.BZ +
                '</a></div>'
              );
            }
          ...
          },
          {
            ...
            formatter: function (row) {
              if (
                (row.ORDER_NUM === '' || row.ORDER_NUM === null) &&
                (row.Batch_Pic_Full_Name === '' ||
                  row.Batch_Pic_Full_Name === null)
              ) {
                return '未上传';
              } else {
                return '已上传';
    ...
    methods: {
      ...
      async returnData(where) {
        ...
        try {
          // 显示确认框
          const confirmResult = await this.$confirm('确认要归还吗？', '提示', {
