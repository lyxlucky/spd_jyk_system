    ...
    <div class="spd-panel__body">
      <el-form size="mini" inline @submit.native.prevent>
        <el-form-item label="品种">
          <el-input v-model="localWhere.varietie" clearable placeholder="编码/全称" style="width: 130px" @keyup.enter.native="search" />
        </el-form-item>
        <el-form-item label="供应商">
          <el-input v-model="localWhere.supplier" clearable style="width: 120px" @keyup.enter.native="search" />
        </el-form-item>
        <el-form-item label="生产批号">
          <el-input v-model="localWhere.batch" clearable style="width: 100px" @keyup.enter.native="search" />
        </el-form-item>
        <el-form-item label="分类属性">
          <el-input v-model="localWhere.classificName" clearable style="width: 100px" @keyup.enter.native="search" />
        </el-form-item>
        <el-form-item label="生产企业">
          <el-input v-model="localWhere.manuEntName" clearable style="width: 110px" @keyup.enter.native="search" />
        </el-form-item>
        <el-form-item label="注册证">
          <el-input v-model="localWhere.approvalNumber" clearable style="width: 110px" @keyup.enter.native="search" />
        </el-form-item>
        ...
        <div class="toolbar-row">
          ...
          <el-button size="mini" @click="showAdvanced = !showAdvanced">
          ...
          <el-button size="mini" @click="syncAndEmit('move-goods', 'unqualified')">移至不合格区</el-button>
        </div>
        <br />
        ...
        <el-form-item label="货位号">
          <el-input v-model="localWhere.position" clearable style="width: 90px" @keyup.enter.native="search" />
        </el-form-item>
        ...
        <el-form-item label="所属区域">
          <el-select v-model="localWhere.upShelfState" clearable placeholder="全部" style="width: 120px">
            <el-option label="全部" value="" />
        ...
        <el-form-item label="条数">
          <el-input-number v-model="localWhere.exportLimit" :min="1" :max="9999" :controls="false" style="width: 70px" />
        </el-form-item>
        ...
        <div class="toolbar-row toolbar-row--secondary">
          ...
          <el-button size="mini" :loading="generating" @click="syncAndEmit('generate-stock')">生成盘点数据</el-button>
          ...
          <el-button size="mini" @click="syncAndEmit('move-goods', 'insulated')">移至普通隔离区</el-button>
          <el-button size="mini" @click="syncAndEmit('update-var-type')">修改物资类型</el-button>
          <el-button size="mini" @click="syncAndEmit('move-goods', 'fangyi')">移至防疫物资区</el-button>
          <el-button size="mini" @click="syncAndEmit('move-goods', 'emergency')">移至应急库区</el-button>
          <el-button v-if="showAdjustDebit" size="mini" @click="syncAndEmit('move-goods', 'adjustDebit')">
            移至盘损隔离区
          </el-button>
          ...
          <el-button size="mini" @click="syncAndEmit('lock-view', 'prelock')">查看定数包预锁</el-button>
        </div>
        <div v-show="showAdvanced" class="advanced-filters">
          ...
          <el-form-item label="高低值">
            <el-select v-model="localWhere.highOrLowClass" style="width: 90px">
              <el-option label="全部" value="-1" />
              <el-option v-for="o in highLowOptions" :key="o.value" :label="o.label" :value="o.value" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="下级分类">
            <el-select v-model="localWhere.highOrLowClassTwo" clearable placeholder="全部" style="width: 110px">
              <el-option label="全部" value="" />
              <el-option v-for="o in highLowTwoOptions" :key="o.value" :label="o.label" :value="o.value" />
            </el-select>
          </el-form-item>
        </div>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    querySourceFrom(query, cb) {
      const list = ['院内中心库', ...(this.sourceFromList || []).map((d) => d.Dept_Two_Name || d)];
...
.dot-expired { background: #ff0000; border-color: #ff0000; }
.dot-15 { background: rgba(255, 165, 0, 0.5); border-color: #ffa500; }
.dot-90 { background: rgba(255, 255, 0, 0.5); border-color: #ffaa00; }
