    ...
    <el-card shadow="never">
      ...
      <el-form class="form-box" :inline="true">
        ...
        <el-form-item label="采购数量:">
          <el-select size="mini" v-model="where.CountState" id="PrchaseDataV2_CountState" style="height:30px;font-size:12px;float:left;" onchange="reloadlrJykOrderTable1Table1()">
        ...
        <el-form-item label="结束日期:">
          <el-select size="mini" v-model="where.last" id="PrchaseDataV2_last" style="height:30px;font-size:12px;float:left;" onchange="reloadlrJykOrderTable1Table1()">
...
<script>
import {
  getPROD_REGISTRATION_LIMIT_BUYV2List,
  InitVarLimitBuyXMClassV2
} from '@/api/Statistics/NewPrchaseDataV2';
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          formatter: (row, column, value) => {
            ...
            // 计算剔除边界的天数
            const betweenDates =
              this.calculateDateDifferenceExcludingBoundaries(
                row.BUY_TIME,
                currentDate
              ); // 开始到今天
            const betweenStartAndEndDates =
              this.calculateDateDifferenceExcludingBoundaries(
                row.BUY_TIME,
                row.BUY_TIME2
              ); // 开始到结束
  ...
  methods: {
    ...
    datasource({ page, limit, where, order }) {
      ...
      where = this.where;
      let data = getPROD_REGISTRATION_LIMIT_BUYV2List({
        page,
        limit,
        where,
        order
