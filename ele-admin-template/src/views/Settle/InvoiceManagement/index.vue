    ...
    <el-card shadow="never">
      ...
      >
        ...
        <template v-slot:MONTHBILLNUM="{ row }">
          ...
          <el-link
            target="_blank"
            @click="ShowMONTHBILLNUM(row.MONTHBILLNUM)"
            >{{ row.MONTHBILLNUM }}</el-link
          >
        </template>
        ...
        <template v-slot:PIC_URL="{ row }">
          <div style="display: flex; justify-content: center">
            ...
            <a
              ...
              :href="BACK_BASE_URL + '/Upload/InvoicePic/' + row.PIC_URL"
              target="_blank"
              lay-event="InovoIcPicPdf"
              >pdf文件</a
            >
            <img
              style="width: 20px; height: 20px"
              v-else-if="row?.PIC_URL != undefined"
              @click="showImage(row.PIC_URL)"
...
<script>
  ...
  import { B2B_BASE_URL, BACK_BASE_URL } from '@/config/setting';
  import {
