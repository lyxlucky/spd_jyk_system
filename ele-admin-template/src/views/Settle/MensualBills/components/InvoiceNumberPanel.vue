    ...
    <div class="panel-grid">
      <div class="col-bill">
        <el-card shadow="never">
          ...
          <el-table v-loading="billLoading" :data="billRows" border stripe size="mini" height="calc(100vh - 240px)" highlight-current-row @current-change="onBillSelect">
          ...
          <el-pagination small layout="total, prev, pager, next" :total="billTotal" :page-size="billPage.size" :current-page.sync="billPage.page" style="margin-top: 8px" @current-change="loadBillList" />
        </el-card>
      </div>

      <div class="col-supplier">
        <el-card shadow="never">
          <template slot="header"><span class="sub-title">供应商</span></template>
          <div class="tool-row">
            ...
            <el-button size="mini" type="primary" icon="el-icon-search" @click="loadSupplierList(1)" />
            <span class="tip">注意:勾选过的号码才会发送HRP</span>
          </div>
          <div class="tool-row">
            <el-button size="mini" type="primary" :disabled="!selectedBill" @click="invPrintOutbound">打印供应商销售出库单</el-button>
            <el-button size="mini" type="primary" :disabled="!selectedBill" @click="invPrintOutboundSummary">打印供应商销售出库单(汇总)</el-button>
            <el-button v-if="hp === 'lg' || hp === 'nyd'" size="mini" @click="invPrintGysSummary">打印供应商汇总统计表</el-button>
            <el-button v-if="hp === 'lg' || hp === 'nyd'" size="mini" @click="invPrintGysDetail">打印供应商供货明细表</el-button>
          </div>
          <div v-if="showJcSelect" class="tool-row">
            <el-select v-model="isJc" size="mini" placeholder="集采" style="width: 120px">
          ...
          <div v-if="hp === 'szlhfy'" class="tool-row">
            <el-button size="mini" @click="invExcelGZSupOutbound">妇幼出库单(供应商送货章)</el-button>
        ...
        <el-card shadow="never">
          <template slot="header">
            <div class="inv-header">
              <span class="sub-title">发票号码</span>
              <div>
                <el-button size="mini" type="primary" :disabled="!canBillApprove" @click="invBillApprove">月结审批</el-button>
                <el-date-picker v-model="fpMonth" type="month" size="mini" value-format="yyyy-MM" placeholder="选择月结月份更新发票" style="width: 180px; margin-left: 8px" />
                <el-button size="mini" style="margin-left: 6px" @click="onUpWxtFph">更新发票(微讯通)</el-button>
              </div>
            </div>
          </template>
...
<style scoped>
.invoice-panel { min-height: calc(100vh - 180px); }
.panel-grid { display: flex; gap: 8px; }
.col-bill { width: 22%; min-width: 240px; }
.col-supplier { flex: 1; min-width: 0; }
.col-invoice { width: 22%; min-width: 240px; }
