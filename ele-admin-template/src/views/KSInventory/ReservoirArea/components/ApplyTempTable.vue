<template>
  <div class="ele-body">
    ...
    <!-- 编辑弹窗 -->
    <EditDialog @refresh="reload" :visible.sync="editDialogVisible"></EditDialog>
  </div>
</template>
...
<script>
  ...
  } from '@/api/KSInventory/ReservoirArea';
  export default {
    name: 'ApplyTempTable',
    components: {
      ApplyTempSearch,
    ...
    methods: {
      ...
      datasource({ page, limit, where, order }) {
        let data = GetDeptTwoRegion({ page, limit, where, order }).then(
          (res) => {
            var tData = {
              count: res.total,
              list: res.result
      ...
      editTempletName(data) {
        this.$bus.$emit('SurgerySchedulingEditTemplateName',data);
