<template>
  <div class="ele-body">
    ...
    <ele-pro-table ref="table" height="60vh" highlight-current-row :stripe="true" :rowClickChecked="true" :rowClickCheckedIntelligent="false" :pageSize="pageSize" :pageSizes="pageSizes" :columns="columns" :datasource="datasource" :selection.sync="selection" @selection-change="onSelectionChange" cache-key="ApplyTempDataTable">
      <!-- 表头工具栏 -->
      <!-- 右表头 -->
      ...
      <template v-slot:APPLY_QTY="{ row }">
        <el-input v-model="row.APPLY_QTY" :min="0" :max="99999999" :step="1" size="mini" />
      </template>

      <template v-slot:JP_APPLY_QTY="{ row }">
        <el-input v-model="row.JP_APPLY_QTY" :min="0" :max="99999999" :step="1" size="mini" />
      </template>
...
import { GetNaxtDayApplyPlanDelTemp, QueryDeptTwoRegionVar } from '@/api/KSInventory/ReservoirArea';
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          minWidth: 150,
          formatter: (row) => {
            const a = row.SPECIFICATION_OR_TYPE2 || '';
            const b = row.SPECIFICATION_OR_TYPE3 || '';
  ...
  methods: {
    ...
    datasource({ page, limit, where, order }) {
      // where.MAIN_ID = this.ApplyTempTableData.ID;
      where.REGION_CODE = this.ApplyTempTableData.REGION_CODE;
      console.log(this.ApplyTempTableData);
      console.log(where);
      let data = QueryDeptTwoRegionVar({ page, limit, where, order }).then(
        (res) => {
          var tData = {
            count: res.total,
            list: res.data
