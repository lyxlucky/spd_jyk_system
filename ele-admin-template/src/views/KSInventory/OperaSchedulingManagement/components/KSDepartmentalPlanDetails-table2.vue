<template>
  <div class="ele-body">
    ...
    >
      ...
      <template v-slot:SKU-NAMES="{ row }">
        <el-select
          :disabled="row.STATE != 0"
          v-model="row.VARIETIE_SKU_NAME"
          placeholder="请选择 SKU"
          clearable
          size="small"
          style="width: 100%"
        >
          <el-option
            v-for="sku in (row.SKU_NAMES || '').split(',')"
            :key="sku.trim()"
            :label="sku.trim()"
            :value="sku.trim()"
          />
      ...
      <template v-slot:toolbar>
        ...
        <naxtDayApplyPlanDelSearch
          @UpNaxtDayApplyPlanDel="UpNaxtDayApplyPlanDelFun"
          @search="reload"
  ...
  export default {
    ...
    methods: {
      ...
      datasource({ page, limit, where, order }) {
        return GetNaxtDayApplyPlanDel({ page, limit, where, order }).then(
          (res) => {
            const listWithSku = res.result.map((item) => ({
              ...item,
            }));
            // 保存到 datasourceList 里
            this.datasourceList = listWithSku;
            // 返回给表格：总数 + 数据
            return {
              count: res.total,
              list: listWithSku
      ...
      UpNaxtDayApplyPlanDelFun() {
        ...
        this.datasourceList.forEach((element) => {
          var data = {
            ...
            APPLY_QTY: element.APPLY_QTY,
            REMARK: element.REMARK,
            SKU_NAME: element.VARIETIE_SKU_NAME
          };
          list.push(data);
        });
