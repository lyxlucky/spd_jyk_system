  ...
  <div class="ele-body">
    ...
    :selection.sync="selection" cache-key="KSInventoryBasicDataTable">
      ...
      <template v-slot:toolbar>
        <!-- 搜索表单 -->
        <KSDepartmentalPlan-search @exportDataExcel="exportDataExcel" @search="reload" @openEdit="openEdit" @addBatchInsertScanDef="addBatchInsertScanDef" :current="current" :type="TYPE" :sumCount="sumCount" />
      ...
      <template v-slot:BATCH_VALIDITY_PERIOD="{ row }">
        ...
        <el-tag v-if="row.COUNT >= 1 && dateNow4 <= row.BATCH_VALIDITY_PERIOD" type="success">{{
            new Date(row.BATCH_VALIDITY_PERIOD)
              .toLocaleDateString()
              .replace('/', '-')
              .replace('/', '-')
