  ...
  <div class="ele-body">
    ...
    >
      ...
      <template v-slot:BATCH_VALIDITY_PERIOD="{ row }">
        ...
        <el-tag
          v-if="
            row.COUNT >= 1 && dateNow1 <= row.BATCH_VALIDITY_PERIOD < dateNow2
          "
          type="danger"
          >{{
        ...
        <!-- <el-tag v-else-if="
            row.COUNT >= 1 && dateNow1 <= row.BATCH_VALIDITY_PERIOD < dateNow5
          " type="danger">{{
            new Date(row.BATCH_VALIDITY_PERIOD)
              .toLocaleDateString()
              .replace('/', '-')
        ...
        <el-tag
          v-else-if="
            row.COUNT >= 1 && dateNow2 <= row.BATCH_VALIDITY_PERIOD < dateNow3
          "
          type="warning"
          >{{
            new Date(row.BATCH_VALIDITY_PERIOD)
        ...
        >
        <el-tag
          v-else-if="row.COUNT >= 1 && dateNow4 <= row.BATCH_VALIDITY_PERIOD"
          type="success"
          >{{
            new Date(row.BATCH_VALIDITY_PERIOD)
              .toLocaleDateString()
      ...
      <template v-slot:DEF_NO_PKG_CODE="{ row }">
        <el-tag
          v-if="row.COUNT >= 1 && row.DEF_NO_PKG_CODE != null"
          type="success"
          >{{ row.DEF_NO_PKG_CODE }}</el-tag
        >
        <el-tag
          v-if="row.COUNT < 1 && row.DEF_NO_PKG_CODE != null"
          type="info"
  ...
  export default {
    ...
    data() {
      return {
        ...
        dateNow5: dayjs().format('YYYY-MM-DD'),
    ...
    methods: {
      isExpiringWithin15Days(validityPeriod) {
        ...
        // 判断是否在当前日期和未来七天之间
        return batchDate.isAfter(now) && batchDate.isBefore(future7Days);
      },
      /* 表格数据源 */
      datasource({ page, limit, where, order }) {
        ...
        let data = GetJykMainShelf({ page, limit, where, order }).then(
          (res) => {
            var tData = {
              count: res.total,
              list: res.result
      ...
      addBatchInsertScanDef() {
        ...
        for (let index = 0; index < this.selection.length; index++) {
          data.DEP_STR += this.selection[index].DEF_NO_PKG_CODE + ',';
        }
        data.DEP_STR = data.DEP_STR.substring(0, data.DEP_STR.length - 1);
        console.log(data);
        BatchInsertScanDef2(data)
          .then((res) => {
            if (res.code == 200) {
              this.$message.success(res.msg);
              this.$emit('search');
