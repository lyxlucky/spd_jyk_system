                ...
                <el-form class="ele-form-search" @keyup.enter.native="search" @submit.native.prevent>
                  <el-row>
                    <el-col v-bind="styleResponsive ? { lg: 10, md: 4 } : { span: 6 }">
                      <el-form-item>
                        <el-input clearable v-model="where.SerachName" placeholder="请输入品种名称/型号规格/生产企业搜索" />
                      </el-form-item>
                    </el-col>
  ...
  methods: {
    ...
    submitItem() {
      this.$confirm('确定要提交所选品种吗?', '提示', {
        ...
        .then(() => {
          addTemplateItem(this.rightSelection)
            .then((res) => {
              this.updateVisible();
    ...
    leftDatasource({ page, limit, where, order }) {
      let data = getLeftTableData({ page, limit, where, order }).then((res) => {
        var tData = {
          count: res.total,
          list: res.result
        };
