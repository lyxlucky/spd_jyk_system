<template>
  <el-dialog title="科室汇总" :visible.sync="innerVisible" width="90%" top="5vh" append-to-body @open="onOpen">
    ...
    <ele-pro-table
      ...
      cache-key="KSConsumeQueryByDeptKsHz_v1"
    />
  </el-dialog>
</template>
...
import { searchDeptHzKs } from '@/api/KSInventory/KSConsumeQueryByDept';
import { getKsHzColumns } from '../columns';
import { isHideDeptSup } from '../utils';

export default {
  name: 'KSConsumeQueryByDeptKsHzDialog',
  props: {
    visible: Boolean,
  ...
  data() {
    return {
      columns: getKsHzColumns(isHideDeptSup()),
      exporting: false
    };
  },
  computed: {
  ...
  methods: {
    ...
    async datasource({ page, limit }) {
      ...
      try {
        const res = await searchDeptHzKs(this.where, page, limit, this.sort, this.deptTwoJson);
    ...
    async exportExcel() {
      this.exporting = true;
      try {
        const res = await searchDeptHzKs(this.where, 1, 9999999, this.sort, this.deptTwoJson);
        ...
        utils.book_append_sheet(book, sheet, '科室汇总');
