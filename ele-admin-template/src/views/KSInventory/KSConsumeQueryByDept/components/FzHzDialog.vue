<template>
  <el-dialog title="消耗分组汇总" :visible.sync="innerVisible" width="90%" top="5vh" append-to-body @open="onOpen">
    ...
    <ele-pro-table
      ...
      cache-key="KSConsumeQueryByDeptFzHz_v1"
    />
  </el-dialog>
</template>
...
import { searchDeptHzFz } from '@/api/KSInventory/KSConsumeQueryByDept';
import { getFzHzColumns } from '../columns';
import { isHideDeptSup } from '../utils';

export default {
  name: 'KSConsumeQueryByDeptFzHzDialog',
  props: {
    visible: Boolean,
  ...
  data() {
    return {
      columns: getFzHzColumns(isHideDeptSup()),
      exporting: false
    };
  },
  computed: {
  ...
  methods: {
    ...
    async datasource({ page, limit }) {
      ...
      try {
        const res = await searchDeptHzFz(this.where, page, limit, this.sort, this.deptTwoJson);
    ...
    async exportExcel() {
      this.exporting = true;
      try {
        const res = await searchDeptHzFz(this.where, 1, 9999999, this.sort, this.deptTwoJson);
        ...
        utils.book_append_sheet(book, sheet, '分组汇总');
