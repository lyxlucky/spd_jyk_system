    ...
    <el-row :gutter="10">
      <el-col v-bind="styleResponsive ? { lg: 4, md: 12 } : { span: 6 }">
        <el-form-item label="品种：" label-width='60px'>
          <el-input clearable v-model="where.VARIETIE_SEARCH_VALUE" placeholder="" />
        </el-form-item>
      </el-col>
      <el-col v-bind="styleResponsive ? { lg: 4, md: 12 } : { span: 6 }">
        <el-form-item label="单号：" label-width='60px'>
          <el-input clearable v-model="where.PLAN_NUMBER" placeholder="" />
        </el-form-item>
      </el-col>
      <el-col v-bind="styleResponsive ? { lg: 4, md: 4 } : { span: 4 }">
        <div class="block">
          <el-form-item label="计划收货日期:" label-width='100px'>
            <el-date-picker v-model="where.PLAN_TIME_START" type="date" style="width:140px" value-format="yyyy-MM-dd" placeholder="yyyy-MM-dd">
            </el-date-picker>
          </el-form-item>
        </div>
      </el-col>
      <el-col v-bind="styleResponsive ? { lg: 3, md: 3 } : { span: 3 }">
        <div class="block">
          <el-date-picker v-model="where.PLAN_TIME_END" type="date" style="width:140px" value-format="yyyy-MM-dd" placeholder="yyyy-MM-dd">
          </el-date-picker>
        </div>
      </el-col>
      <el-col v-bind="styleResponsive ? { lg: 7, md: 12 } : { span: 6 }">
        <el-form-item label="收货状态:" label-width='80px'>
          <el-select v-model="where.STATE" style="width:100px" @change="search()">
