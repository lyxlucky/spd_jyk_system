  ...
  <div class="ele-body">
    ...
    >
      ...
      <template v-slot:BATCH_VALIDITY_PERIOD="{ row }">
        ...
        >
        <el-tag
          v-if="row.COUNT >= 1 && dateNow4 <= row.BATCH_VALIDITY_PERIOD"
          type="success"
          >{{
            new Date(row.BATCH_VALIDITY_PERIOD)
              .toLocaleDateString()
      ...
      <template v-slot:action="{ row }">
        <el-button
          v-if="row.COUNT >= 1 && row.DEF_NO_PKG_CODE == null && TYPE != ''"
          size="small"
          type="primary"
          icon=""
          class="ele-btn-icon"
  ...
  export default {
    ...
    methods: {
      ...
      datasource({ page, limit, where, order }) {
        ...
        let data = GetJykMainShelfHz({ page, limit, where, order }).then(
          (res) => {
            var tData = {
              count: res.total,
              list: res.result
