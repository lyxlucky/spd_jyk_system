...
import StockmaintenanceSearch from './components/StockmaintenanceSearch.vue';
import {
  queryStockMaintenance,
  exportStockMaintenanceExcel,
  updateRemarks,
  updateSendSupplierState
} from '@/api/KSInventory/Stockmaintenance';
import {
  openExcelFile,
  fmtDate10,
  fmtPrice,
  fmtStorageId,
  fmtUpShelfStateMaintenance,
  isBatchExpired,
  normalizeRowForPost
} from './utils';

export default {
  name: 'KSInventoryStockmaintenance',
  components: { StockmaintenanceSearch },
  data() {
    return {
  ...
  methods: {
    ...
    isBatchExpired,
    tableRowClass({ row }) {
      return isBatchExpired(row.BATCH_VALIDITY_PERIOD) ? 'stock-maintenance-row-expired' : '';
    },
    ...
    async datasource({ page, limit, where, order }) {
      ...
      this.lastSort = sort;
      try {
        const res = await queryStockMaintenance(w, page, limit, sort);
        this.summary = {
          avgday: res.avgday ?? '',
          stockPosition: res.stockPosition ?? ''
        };
    ...
    async onExport() {
      ...
      try {
        ...
        const res = await exportStockMaintenanceExcel(where, this.lastSort);
