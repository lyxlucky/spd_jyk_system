...
<script>
  import {
    ...
    apiGetLoadGeneratedVarietieInfo,
    apiGetLoadGoodsDeliveryNumbers,
    apiGetSearchDeliveryVarietie,
    apiGetSearchTbMainZy,
    apiPostCreateDefTb,
    apiPostGtSbkSp,
    apiPostSearchTbMainZy,
    apiPostUploadGtPic,
    apiPostNurseApprove
  } from '@/api/HeelBlockConsumables/ConsumeApprove';
    ...
    methods: {
      ...
      async getLoadGeneratedVarietieInfo() {
        ...
        let where = {
          ...
          deliveryNumber: this.currentConsumeA1
            ? this.currentConsumeA1.Delivery_Note_Number
            : ''
        };
      ...
      onNurseApproval(e) {
        ...
        console.log(this.currentConsumeA1);
        this.$confirm(
          `确认审批收货单号 [${this.currentConsumeA1.Delivery_Note_Number}] 吗`,
          '提示',
          {
      ...
      onGeneratePackageSure(e) {
        ...
        // TB_ID: 43
        let where = {
          orderNum: this.currentConsumeA1
            ? this.currentConsumeA1.Delivery_Note_Number
            : '',
      ...
      async submitUpload() {
        ...
        let where = {
          Delivery_Note_Number: this.currentConsumeA1
            ? this.currentConsumeA1.Delivery_Note_Number
            : '',
          FILE: fileData,
          filename: file.name
      ...
      isHvaeChargCode() {
        ...
        if (
          !this.DeliveryVarietieList ||
          this.DeliveryVarietieList.length == 0
        ) {
        ...
        let deliveryNumberId = this.currentConsumeA1
          ? this.currentConsumeA1.Delivery_Note_Number
          : '';
        let where = {
          ...
          .then((res) => {
            ...
            if (data.code == 200) {
              if (HOME_HP == 'zq' || HOME_HP == 'stzx') {
                ...
                window.open(
                  BACK_BASE_URL +
                    '/api/B2BVarietieConsumeApprove/GetTags?id=5&format=pdf&inline=true&json=' +
                    where.Batch_Id +
                    '&deliveryNumberId=' +
                    deliveryNumberId +
                    '&title=' +
