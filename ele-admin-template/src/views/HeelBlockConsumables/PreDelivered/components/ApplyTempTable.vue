<template>
  <div class="ele-body">
    ...
    <!-- 数据表格 -->
    <ele-pro-table highlight-current-row @current-change="onCurrentChange" :row-class-name="tableRowClassName" ref="table" height="60vh" :rowClickChecked="true" :stripe="false" :pageSize="pageSize" :pageSizes="pageSizes" :columns="columns" :needPage="false" :datasource="datasource" :selection.sync="selection" cache-key="ApplyTempTable">
      <!-- 表头工具栏 -->
...
import { LoadGoodsDeliveryNumbers } from '@/api/HeelBlockConsumables/PreDelivered';
...
export default {
  ...
  methods: {
    ...
    datasource({ page, limit, where, order }) {
      where.deptTwoJson = this.$store.state.user.info.DeptNow.Dept_Two_Code;
      where.UserId = this.$store.state.user.info.ID;
      let data = LoadGoodsDeliveryNumbers({ page, limit, where, order }).then((res) => {
        var tData = {
          count: res.total,
          list: res.result
        };
