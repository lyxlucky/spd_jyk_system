<template>
  <div class="ele-body spd-page yg-var-cl-look">
    <el-card shadow="never">
      ...
      <el-form size="small" inline class="filter-block" @submit.native.prevent>
        ...
        <el-form-item>
          ...
          <el-button type="primary" plain icon="el-icon-download" :loading="exporting" @click="exportExcel">
            导出
          </el-button>
          <el-button
            v-if="isSzhn"
...
import { getYbClInfo, sendVarDataIdSZHN } from '@/api/Home/YgVarClLook';
import {
  defaultLookFilters,
  LOOK_WJ_AUTH_JSON,
...
export default {
  ...
  methods: {
    ...
    datasource({ page, limit, where }) {
      const merged = this.buildQuery(where);
      const { WJ_AUTH, ...filters } = { ...merged, WJ_AUTH: LOOK_WJ_AUTH_JSON };
    ...
    async exportExcel() {
      ...
      try {
        ...
        exportYgVarClLookExcel(res.result || []);
    ...
    async onSendSzhnHis() {
      ...
      try {
        const res = await sendVarDataIdSZHN(this.selection);
