      ...
      <el-row :gutter="12">
        <el-col v-bind="styleResponsive ? { sm: 20 } : { span: 20 }">
          <el-form-item label="备注:" prop="WJ_SP_MARK">
            <el-input size="mini" clearable v-model="form.WJ_SP_MARK" placeholder="备注" />
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
<script>
import { ApproveSpdMainsjLinesIface } from '@/api/Home/BJRMPriceAudit';
export default {
  components: {},
  props: {
    // 弹窗是否打开
  ...
  methods: {
    save() {
      this.$refs.form.validate((valid) => {
        ...
        this.form.WJ_SP_MAN = this.$store.state.user.info.Nickname;
        const loading = this.$messageLoading('请求中..');
        ApproveSpdMainsjLinesIface(this.form)
          .then((res) => {
            this.$emit('done');
