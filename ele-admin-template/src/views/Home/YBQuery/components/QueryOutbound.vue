    ...
    <div class="yb-query-outbound-table">
      ...
      >
        <template #state="scope">
          <div :class="stateColorMap[scope.row.ybstate_bg]">{{
            stateMap[scope.row.ybstate_bg]
          }}</div>
        </template>
      </ele-pro-table>
    </div>
...
<script>
  ...
  import { GetYB3502CKList, ExportYB3502CK, PrintYBApiLog } from '@/api/Home/YBQuery';
  ...
  export default {
    ...
    methods: {
      ...
      async handleExport(form) {
        ...
        const res = await ExportYB3502CK({
          ...form,
