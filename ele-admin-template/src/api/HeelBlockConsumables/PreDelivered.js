...
export async function SearchDeliveryVarietie(data) {
  ...
  data2.page = data.page;
  data2.size = data.limit;
  data2.deliveryNumberId = data.where.deliveryNumberId
    ? data.where.deliveryNumberId
    : '';
  data2.condition = data.where.condition ? data.where.condition : '';
...
export async function SoftDeleteDeliveryNumber(data) {
  ...
  const res = await request.post(
    '/B2BPreTakeDelivery/SoftDeleteDeliveryNumber',
    rep
  );
  if (res.data.code == 200) {
    return res.data;
...
export async function GetTwoDept() {
  const res = await request.get('/ZNG/GetTwoDept');
  if (res.data.code == 200) {
    return res.data;
  } else {
    return Promise.reject(new Error(res.data.msg));
...
export async function upDeliveryInfo(data) {
  const formatData = {
    Token: sessionStorage.getItem(TOKEN_STORE_NAME),
    deliveryNumberId: data?.deliveryNumberId,
    B2B_SPD_DEPT_TWO_CODE: data?.B2B_SPD_DEPT_TWO_CODE,
    HOSPITALIZATION_NUMBER: data?.HOSPITALIZATION_NUMBER,
    PATIENT: data?.PATIENT
  };
  var req = formdataify(formatData);
  const res = await request.post('/B2BPreTakeDelivery/upDeliveryInfo', req);
  if (res.data.code == 200) {
    return res.data;
  } else {
    return Promise.reject(new Error(res.data.msg));
