...
export async function GetSpdMainsjHeaderIface(data) {
  ...
  formataData.APPLYPEOPLE = data.where.APPLYPEOPLE
    ...
    : '';
  formataData.YB_SP_STATE = data.where.YB_SP_STATE
    ? data.where.YB_SP_STATE
    : '';
  formataData.WJ_SP_STATE = data.where.WJ_SP_STATE
...
export async function GetSpdMainsjLinesIface(data) {
  ...
  formataData.HEADER_IFACE_ID = data.where.HEADER_IFACE_ID
    ...
    : '-1';
  formataData.REGISTRATION_NAME = data.where.REGISTRATION_NAME
    ? data.where.REGISTRATION_NAME
    : '';
  formataData.REGISTRATION_NO = data.where.REGISTRATION_NO
    ? data.where.REGISTRATION_NO
    : '';
  formataData.ITEM_NUMBER = data.where.ITEM_NUMBER
    ? data.where.ITEM_NUMBER
    : '';
  formataData.STAND_VALUE = data.where.STAND_VALUE
    ? data.where.STAND_VALUE
    : '';
...
export async function ImportSpdMainsjLinesIfaceExcel(data) {
  ...
  const res = await request.post(
    '/AJykDept/ImportSpdMainsjLinesIfaceExcel',
    data
  );
  if (res.data.code == 200) {
    return res.data;
...
export async function BatchPushChargeItemToHis(data) {
  const formData = {
    Token: sessionStorage.getItem(TOKEN_STORE_NAME),
    HeaderIfaceId: data.HEADER_IFACE_ID
  };
  const res = await request.post(
