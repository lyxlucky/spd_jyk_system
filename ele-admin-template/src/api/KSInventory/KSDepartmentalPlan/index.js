...
export async function SearchHistoryConsumedAndPurchaseDept(data) {
    ...
    date = `${date.toLocaleDateString().replace(/\//g, '-')}` + ` ${date.toTimeString().substring(0, 8)}`;
    ...
    const res = await request.post('/PurchaseOrderApply/SearchHistoryConsumedAndPurchaseDept', rep);
    if (res.data.code == 200) {
        return res.data;
    } else {
        return Promise.reject(new Error(res.data.msg));
...
export async function getApplyOperateTip(data) {
    ...
    data2.PLAN_TIME_START = data.where.PLAN_TIME_START ? data.where.PLAN_TIME_START : '';
...
export async function QueryPageLayUIByJYK(data) {
    ...
    data2.HIGH_CLASS_XH_SEARCH = data.where.HIGH_CLASS_XH_SEARCH ? data.where.HIGH_CLASS_XH_SEARCH : '';
